...
uint64 pattern::count_pieces(){
  uint64 count = 0;
  // counting active bits with "Brian Kernighan’s Algorithm"
  while (pieces) {
    pieces &= (pieces - 1);
    count++;
  }
...
void heuristic::get_params_from_array(double* input){
  // Addition: new parameter opening weight
  // Addition: new parameter centrality decay
  ...
  center_decay=input[7];
  for(unsigned int i=0;i<4;i++)
    opening_weight[i]=input[i+8];
  for(unsigned int i=0;i<Nweights;i++)
    w_act[i]=input[i+12];
  for(unsigned int i=0;i<Nweights;i++)
    w_pass[i]=input[i+Nweights+12];
  for(unsigned int i=0;i<Nweights;i++)
    delta[i]=input[i+2*Nweights+12];
  update();
}
...
vector<zet> heuristic::get_moves(board& b, bool player, bool nosort=false){
  ...
  // Addition: use corner weights if it is the first move
  if(b.num_pieces() == 0) {
      for(i=1,m=1;m!=boardend;m<<=1){
          if(uint64totile(m)==0){
              candidate.push_back(zet(m,deltaL+noise(engine)+center_weight*vtile[m]+opening_weight[0],player));
          }
          else if(uint64totile(m)==8){
              candidate.push_back(zet(m,deltaL+noise(engine)+center_weight*vtile[m]+opening_weight[1],player));
          }
          else if(uint64totile(m)==27){
              candidate.push_back(zet(m,deltaL+noise(engine)+center_weight*vtile[m]+opening_weight[2],player));
          }
          else if(uint64totile(m)==35){
              candidate.push_back(zet(m,deltaL+noise(engine)+center_weight*vtile[m]+opening_weight[3],player));
          }
          else{
              candidate.push_back(zet(m,deltaL+noise(engine)+center_weight*vtile[m],player));
          }
          lookup[m]=i;
          i++;
      }
  }
  // Addition: add centrality decay
  else {
      for(i=1,m=1;m!=boardend;m<<=1){
          if(b.isempty(m)){
//               candidate.push_back(zet(m,deltaL+center_weight*vtile[m]+noise(engine),player));
              candidate.push_back(zet(m,deltaL+center_weight*vtile[m]*exp(-center_decay*b.num_pieces())+noise(engine),player));
//               candidate.push_back(zet(m,deltaL+center_weight*vtile[m]*(center_decay/(center_decay+b.num_pieces()))+noise(engine),player));
              lookup[m]=i;
              i++;
          }
      }
