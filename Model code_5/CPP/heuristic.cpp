...
void heuristic::get_params_from_array(double* input){
  ...
  opening_weight=input[7];
  for(unsigned int i=0;i<Nweights;i++)
    w_act[i]=input[i+8];
  for(unsigned int i=0;i<Nweights;i++)
    w_pass[i]=input[i+Nweights+8];
  for(unsigned int i=0;i<Nweights;i++)
    delta[i]=input[i+2*Nweights+8];
  update();
}
...
vector<zet> heuristic::get_moves(board& b, bool player, bool nosort=false){
  ...
  // Addition: use histogram instead of center weight if it is the first move
  if(b.num_pieces() == 0) {
      for(i=1,m=1;m!=boardend;m<<=1){
          candidate.push_back(zet(m,deltaL+noise(engine)+opening_weight*opening_hist[uint64totile(m)],player));
          lookup[m]=i;
          i++;
      }
  }
